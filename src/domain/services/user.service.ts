...
export const uploadUserImageMiddleware = upload.single("image");

export const handleUploadUserImage = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {0
    ...
    const imagePath = await uploadUserImage(req, userId);
...
const uploadUserImage = async (req: Request, userId: string): Promise<string> => {
  if (!req.file) {
  ...
  const updatedUser = await userOdm.uploadUserImage(userId, newPath);
